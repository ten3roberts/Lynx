#pragma once
// Namespace for generating random values, arrays, and vectors. No arguments
// yields a result between 0 and 1
struct Vector2;
struct Vector3;
struct Vector4;
#include <random>
namespace Random
{
	...
	// Will return a vector with a random direction and magnitude. (Equal chance for
	// each magnitude; sparser further out)
	Vector2 InCircle(float outerRadius, float innerRadius = 0);
	Vector2 InCircle();

	// Will return a vector to a random point on a circle's surface. (Unequal chance
	// for each magnitude; even spread throughout whole sphere)
	...
	// for each magnitude; sparser further out)
