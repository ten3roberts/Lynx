...
namespace Lynx
{
	class LYNX_API XMLNode
	{
	  ...
	  public:
		...
		XMLNode(XMLNode* parent, size_t depth, bool valid)
			: m_tag(""), m_content(""), m_children(), m_depth(depth), m_parent(parent), m_valid(parent->isValid())
		{
		}
		...
		std::vector<XMLNode*>* getChildren(size_t index)
		{
		...
		// XMLNode* operator[](const std::string& tag) { if (m_children.find(tag) !=
		// m_children.end()) return m_children[tag][0]; return nullptr; }
		XMLNode& operator[](const std::string& tag)
		{
			if (m_children.find(tag) != m_children.end())
				return *m_children[tag][0];
			m_children[tag] = {new XMLNode(this, m_depth + 1, m_valid)};
			return *m_children[tag][0];
		}
